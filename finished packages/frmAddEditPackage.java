...
public class frmAddEditPackage extends JFrame {
	...
	public frmAddEditPackage(boolean isAdd, Package package1) {
			...
			public void actionPerformed(ActionEvent e) {
				if(isAdd) {
					if(Validation.isPresent(tfPackName) && Validation.isDate(startDate) && Validation.isDate(endDate) && 
						...
						if(((Date)startDate.getModel().getValue()).before((Date)endDate.getModel().getValue())) {
							Package insert = new Package();
							insert.setPkgName(tfPackName.getText());
							insert.setPkgDesc(tfDesc.getText());
							insert.setPkgBasePrice(Double.parseDouble(tfBasePrice.getText()));
							insert.setPkgAgencyCommission(Double.parseDouble(tfComm.getText()));
							...
							JOptionPane.showMessageDialog(frmAddEditPackage.this, "Start Date must be before end date");
						}
					}
					...
					if(Validation.isPresent(tfPackName) && Validation.isDate(startDate) && Validation.isDate(endDate) && 
						...
						if(((Date)startDate.getModel().getValue()).before((Date)endDate.getModel().getValue())) {
							...
							Double basePrice = getDouble(tfBasePrice.getText());
							Double commPrice = getDouble(tfComm.getText());
							newPack.setPkgBasePrice(basePrice);
							newPack.setPkgAgencyCommission(commPrice);
		...
		if(isAdd) {
			...
			String bp = n.format(package1.getPkgBasePrice());
			String cp = n.format(package1.getPkgAgencyCommission());
			
			tfBasePrice.setText(bp);
			tfComm.setText(cp);
			try {
