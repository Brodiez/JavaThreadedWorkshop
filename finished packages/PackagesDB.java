...
public class PackagesDB {
	...
	static NumberFormat n = new DecimalFormat("#.00"); 
	...
	public static DefaultListModel<Package>  getPackages() throws ClassNotFoundException, SQLException {
		...
		try {
			...
			while(rs.next()) {
				...
				p.setPkgBasePrice(rs.getDouble("PkgBasePrice"));
				p.setPkgAgencyCommission(rs.getDouble("PkgAgencyCommission"));
	...
	public static Package getPackage(int PackageId) throws ClassNotFoundException, SQLException {
		...
		try {
			...
			pack.setPkgAgencyCommission(rs.getDouble("PkgAgencyCommission"));
			rs.close();
			return pack;
	...
	public static boolean updatePackage(Package oldPack, Package newPack) throws ClassNotFoundException, SQLException {
		...
		try {
			...
			stmt.setDouble(5, newPack.getPkgBasePrice());
			stmt.setDouble(6, newPack.getPkgAgencyCommission());
	...
	public static DefaultTableModel resultSetToTableModel(ResultSet rs) {
		try {
			...
			if(i==4 || i ==5) {
					newRow.addElement((n.format(rs.getDouble(i))));
					System.out.println(Double.valueOf(n.format( rs.getDouble(i))));
			}
			else
